...
const steps = [
  ...
  },
  {
    ...
    items: [
      ...
      "Установите уровень шума (Noise Level) для имитации легитимного трафика",
      "Нажмите «Start Run» и дождитесь завершения"
    ],
  ...
  {
    num: 5,
    title: "Назначение задания студенту (CTF Mode)",
    icon: Globe,
    color: "red",
    desc: "Как только симуляция завершилась, платформа сохраняет реальные артефакты атаки. Теперь нужно выдать задание.",
    items: [
      "В разделе Simulations найдите завершенную симуляцию",
      "Нажмите иконку Assign to Student (или Manage Assignments)",
      "Выберите зарегистрированного студента из списка",
      "Напишите Legend (Легенду/завязку инцидента)",
      "Нажмите Assign. Студент увидит задание в личном кабинете."
    ],
    note: "Система автоматически сгенерирует ответы (хэши, IP, PID) на основе этой конкретной симуляции."
  },
  {
    num: 6,
    title: "Контроль и проверка",
    icon: CheckCircle,
    color: "teal",
    desc: "Система MARS автоматизирует проверку заданий.",
    items: [
      "Студент проводит расследование в Kibana и сдает найденные артефакты через свой интерфейс",
      "Система автоматически сверяет ответы с реальными данными",
      "В разделе Assignments вы увидите статус: PENDING, COMPLETED (100 баллов) или FAILED (0 баллов)",
      "Вы можете открыть Cheat Sheet (Шпаргалку) у симуляции, чтобы помочь студенту, если он зашел в тупик"
    ],
    note: "Не показывайте шпаргалку аналитикам до окончания расследования — это готовые ответы!"
  }
]
