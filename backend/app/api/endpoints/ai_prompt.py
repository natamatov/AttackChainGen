...
ВНИМАНИЕ: поле `template` в каждом шаге АБСОЛЮТНО ОБЯЗАТЕЛЬНО! Без него сценарий не будет работать. 
КРИТИЧЕСКОЕ ПРАВИЛО: КАТЕГОРИЧЕСКИ ЗАПРЕЩАЕТСЯ выдумывать или галлюцинировать имена шаблонов! Если ты используешь шаблон, которого нет в списке "ДОСТУПНЫЕ ШАБЛОНЫ СОБЫТИЙ", система выдаст критическую ошибку и упадет. Используй СТРОГО только те имена, которые переданы в списке. Если подходящего шаблона нет, используй наиболее близкий по смыслу из доступных!
"""
